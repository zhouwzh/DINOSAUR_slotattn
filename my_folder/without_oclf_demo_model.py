...
class VitTextModel(nn.Module):
    ...
    def __init__(
        ...
        if args.vit_checkpoint:
            ...
            self.vit.load_state_dict(new_state, strict=True)
            msg = self.vit.load_state_dict(new_state, strict=False)
    ...
    def forward(self, images: torch.Tensor, tok_ids: torch.Tensor, tok_lens: torch.Tensor, masks: torch.Tensor):
        ...
        if not self.mask_pool:
            ...
            B,N,C = tok.shape
            S = int(N ** 0.5)
            feat_map = tok.transpose(1,2).reshape(B,C,S,S)
            ...
            z_img = self._l2norm(self.vision_proj(pooled_mean))
