    ...
    for i, batch in enumerate(val_loader):
        if i % 1 == 0:
            if args.use_original_data:
                ...
                input, target_index = build_val_input(batch, args)
                video = input_dict["image"].unsqueeze(1)
                slots, attn_vis, attns = model.steve_model.encode(video)
                frame = steve_visualize(video, attns, 15)
                ...
                print(f"save to /scratch/wz3008/new_SlotAttn/object-centric-learning-framework/my_folder/visualize/test_{i}.jpg")
                import pdb; pdb.set_trace()
                continue
            else:
                #input = {
