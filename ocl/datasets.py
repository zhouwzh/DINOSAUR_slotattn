...
def _load_npz_fields(elem: Dict[str, Any], npz_keys: Tuple[str, ...] = ("instances",)) -> Dict[str, Any]:
    """
    Convert selected fields (e.g. 'instances') from:
      - bytes / bytearray / memoryview  -> dict[str, np.ndarray]
      - np.lib.npyio.NpzFile            -> dict[str, np.ndarray]
    Keep as-is if already dict-like.
    """
    for k in npz_keys:
        if k not in elem:
            continue
        ...
        # already a plain dict (best case)
        if isinstance(v, dict):
            continue

        # np.load returns NpzFile
        if isinstance(v, np.lib.npyio.NpzFile):
            elem[k] = {kk: v[kk] for kk in v.files}
            continue

        # raw bytes -> load via BytesIO
        if isinstance(v, (bytes, bytearray, memoryview)):
            bio = io.BytesIO(bytes(v))
            npz = np.load(bio, allow_pickle=False)
            elem[k] = {kk: npz[kk] for kk in npz.files}
            continue

        # sometimes decoders may return a file-like object
        if hasattr(v, "read"):
            data = v.read()
            bio = io.BytesIO(data)
            npz = np.load(bio, allow_pickle=False)
...
class WebdatasetDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        # MODIFICATION
        extra_keys_to_keep: Tuple[str, ...] = tuple(),
        ...
        # MODIFICATION
        # self.extra_keys_to_keep = extra_keys_to_keep
        # self.force_single_worker = force_single_worker
    ...
    def _create_webdataset(
        ...
        # MODIFICATION: "instances" in keys_to_keep --> {key: np.ndarray}
        # datapipe = datapipe.map(partial(_load_npz_fields, npz_keys=("instances",)))
    ...
    def _create_dataloader(self, dataset, batch_transforms, size, batch_size, partial_batches, num_workers=None):
        ...
        # MODIFICATION
        # nw = 0 if self.force_single_worker else self.num_workers
        # dataloader = DataLoader(dataset, num_workers=nw, batch_size=None)
    ...
    def train_data_iterator(self):
        ...
        return self._create_webdataset(
            ...
            # keys_to_keep=tuple(set(_collect_fields(transforms) + self.extra_keys_to_keep)),
